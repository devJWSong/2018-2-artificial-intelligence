#lang racket
(define (Depth queue final max num limit stop)
  (cond
    ((null? queue) max)
    ((LoadSame? (car queue) final 0) num)
    ((Overload? (car queue)) (Depth (cdr queue) final max (+ 1 num) limit stop))
    ((and (eq? stop #f) (eq? (list-ref (car queue) 6) limit)) (Depth queue final max num limit #t))
    ((eq? stop #f) (Depth (FrontAddChildren (cdr queue) (car queue) 0) final (ReturnBigger (car queue) max) (+ 1 num) limit stop))
    (else (Depth (cdr queue) final (ReturnBigger (car queue) max) (+ 1 num) limit stop))
    )
  )

(define (Breath queue final max num)
  (cond
    ((null? queue) max)
    ((LoadSame? (car queue) final 0) num)
    ((Overload? (car queue)) (Breath (cdr queue) final max (+ 1 num)))
    (else (Breath (BackAddChildren (cdr queue) (car queue) 0) final (ReturnBigger (car queue) max) (+ 1 num)))
    )
  )

(define (Iterative queue final max num iter allowed)
  (cond
    ((< allowed iter) max)
    ((list? (Depth queue final max num iter #f)) (Iterative (FrontAdd '() (list '(0 0 0 10 20 15 0))) final (Depth queue final max num iter #f) 0  (+ 1 iter) allowed))
    (else (Depth queue final max num iter #f))
    )
  )
...
; return bigger loaded list
(define (ReturnBigger x y)
  (if (LoadBigger? x y 0) x y)
  )
...
(Depth (FrontAdd queue (list initial)) final initial 0 15 #f)
(Breath (FrontAdd queue (list initial)) final initial 0)
(Iterative (FrontAdd queue (list initial)) final initial 0 0 allowed)
